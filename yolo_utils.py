...
def letterbox_image(image, size):
    ...
    w, h = size
    new_w = int(image_w * min(1.0*w/image_w, 1.0*h/image_h))
    new_h = int(image_h * min(1.0*w/image_w, 1.0*h/image_h))
    print new_h,new_w
        ...
        for t, n in enumerate(best_anchor):
            for l in range(3):  # 1 in 3 scale
                if n in anchor_mask[l]:  # choose the corresponding mask: best_anchor in [6, 7, 8]or[3, 4, 5]or[0, 1, 2]
                    ...
                    i = np.floor(true_boxes[b, t, 0] * grid_shapes[l][1]).astype(np.int32)  #ex: 3+1.2=4.2--> vao o co y=4
